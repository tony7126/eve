...
def get(resource, lookup):
    ...
    if config.DOMAIN[resource]["aggregation"]:
        find_method = app.data.aggregate#(resource, req)
    else:
        find_method = app.data.find
    ...
    if req.if_modified_since:
        ...
        cursor = find_method(resource, preflight_req, lookup)
        if cursor.count() == 0:
    ...
    cursor = find_method(resource, req, lookup)
